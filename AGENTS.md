...
- **超星登录 `session_failed` ≠ 浏览器问题，根因是数据库 schema 落后于代码**：`/auth/error?reason=session_failed` 是"超星身份已验证、但本地 Supabase 会话建立失败"的统一兜底错误，与浏览器/访客模式无关，**所有走完整超星 OAuth 的新登录都会命中**。真实根因是 `supabase/migrations/*.sql` 没有按顺序在两个环境（develop/product）全部执行，代码引用的新列/新表在数据库里不存在（典型报错 `PGRST204 Could not find the ... column`）。**新增迁移后，必须确认对应 `ALTER TABLE ... ADD COLUMN` / `CREATE TABLE` 已在 develop 和 product 两个库都落地，才能让代码引用这些列/表**；登录写入链路顺序为 `profiles` → `enrollments` → `external_identities`，`profiles` 写失败会连锁导致后两者外键失败。本次缺失并已由 `supabase/migrations/202609080001_repair_auth_profile_schema.sql`（幂等，含 `IF NOT EXISTS`/`DROP POLICY IF EXISTS`）补齐：`profiles` 的 `major_name/grade_name/class_name/academic_source` 四列、`student_experiment_profiles` 表及其两条 RLS policy、`profiles_academic_dimensions_idx`/`one_in_progress_quiz_per_student_step_idx`/`quiz_questions_step_text_unique_idx` 三个索引。develop 已执行验证；**product 库只读（exec_sql 仅 SELECT），需通过平台迁移/发布流程执行同文件才能修生产环境**。
