...
function previewQuizQuestions(step: ExperimentStep) {
    ...
    return Array.from({
        ...
        options: [{
            id: "A",
            text: "说明因果原理，并记录关键参数、对照与结果判断"
        }, {
            id: "B",
            text: "只写操作名称，不记录条件和结果"
        }, {
            id: "C",
            text: "只描述看到的现象，不分析误差来源"
        }, {
            id: "D",
            text: "省略对照，直接给出结论"
        }]
...
export default function StepWorkstation(
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (!cancelled)
                    setError(clientErrorMessage(err, "加载题目失败"));
            } finally {
                if (!cancelled)
                    setQuizLoading(false);
            }
    ...
    useEffect(() => {
        if (typeof window === "undefined" || loadedDraft.current)
            return;
        ...
        try {
            ...
            if (saved)
                setDescs(JSON.parse(saved) as Record<string, string>);
        } catch {}
    }, [draftKey]);
    ...
    async function submitQuizAnswer() {
        if (!quizSelectedOption || !quizSessionId || quizQuestions.length === 0)
            return;
        ...
        const newAnswers = {
            ...quizAnswers,
            [currentQuestion.question_id]: quizSelectedOption
        };
        ...
        <div
            ...
            <div
                ...
                <main className="student-stage-canvas min-w-0">
                    ...
                    {stage === 0 && <section className="student-stage-panel student-stage-panel-task mt-4">
                        <div
                            className="rounded-2xl bg-primary-container/40 border border-primary/20 p-5">
                            <p
                                className="text-xs font-black text-primary bg-card/80 border border-primary/30 rounded-full px-2.5 py-1 inline-flex items-center gap-1"><BookOpen className="w-3 h-3" />引导级 · 任务情境</p>
                        ...
                        <div
                            className="rounded-2xl bg-card/90 border border-secondary/30 shadow-card p-5 mt-5">
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                <span
                                    className="ml-auto shrink-0 text-xs font-bold text-secondary bg-secondary-container border border-secondary/30 rounded-full px-2.5 py-1">核心机制</span>
                            </div>
                            <div className="mt-3 grid gap-3">
                                {principlePoints.map((point, index) => <div key={index} className="flex gap-3">
                                    <span
                                        className="shrink-0 w-6 h-6 rounded-full bg-secondary-container text-secondary text-xs font-black flex items-center justify-center">{index + 1}</span>
                        ...
                        <div className="grid lg:grid-cols-2 gap-5 mt-5">
                            {(() => {
                                ...
                                return (
                                    <>
                                        <div
                                            className="rounded-2xl bg-card/90 border border-border/60 shadow-card p-5">
                                            ...
                                            <ul className="mt-3 space-y-2 text-xs leading-relaxed">
                                                {knowledgePoint.hints.map(
                                                    (hint, index) => <li key={hint} className="flex gap-2"><span className="text-primary font-black">{index + 1}</span><span>{hint}</span></li>
                                                )}
                                            </ul>
                                        </div>
                                        <div
                                            ...
                                            <ul className="mt-3 space-y-2 text-xs leading-relaxed">
                                                {decisionPoint.hints.map(
                                                    (hint, index) => <li key={hint} className="flex gap-2"><span className="text-secondary font-black">{index + 1}</span><span>{hint}</span></li>
                                                )}
                        ...
                        <div
                            className="execution-details-heading mt-5 flex items-center justify-between gap-3">
                            <p className="text-xs font-black text-primary">实验执行要点 · 四项同步显示</p>
                            <button
                                type="button"
                                onClick={() => setShowExecutionDetails(current => !current)}
                                className="inline-flex min-h-10 items-center gap-1.5 rounded-xl border border-primary/20 bg-card px-3 text-xs font-bold text-primary">
                                {showExecutionDetails ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}{showExecutionDetails ? "收起全部" : "展开全部"}
                            </button>
                        </div>
                        {showExecutionDetails && <div
                            className="execution-details-grid grid md:grid-cols-2 xl:grid-cols-4 gap-4 mt-3">
                            {[{
                                title: "SOP 参数",
                                items: step.sopParameters,
                                tone: "text-primary"
                            }, {
                                title: "安全事项",
                                items: step.safetyNotes,
                                tone: "text-warning"
                            }, {
                                title: "判断与排错",
                                items: step.decisionTree,
                                tone: "text-secondary"
                            }, {
                                title: "设备与记录",
                                items: [...step.instruments, step.scientificPractice],
                                tone: "text-success"
                            }].map(group => <article
                                key={group.title}
                                className="rounded-2xl bg-card/90 border border-border/60 shadow-card p-4">
                                <h3 className={`text-sm font-extrabold ${group.tone}`}>{group.title}</h3>
                                <ul className="mt-3 space-y-2 text-xs leading-relaxed text-muted-foreground">
                                    {group.items.map(
                                        item => <li key={item} className="flex gap-2"><span>•</span><span>{item}</span></li>
                                    )}
                                </ul>
                            </article>)}
                        </div>}
                    ...
                    {stage === 1 && <>
                        <section
                            className="mt-4 rounded-2xl bg-primary-container/40 border border-primary/20 p-5">
                            <p
                                className="text-xs font-black text-primary bg-card/80 border border-primary/30 rounded-full px-2.5 py-1 inline-flex items-center gap-1"><CircleHelp className="w-3 h-3" />引导级 · 知识检验</p>
                        ...
                        {quizLoading && <section
                            className="mt-5 rounded-2xl bg-card/90 border border-border/60 shadow-card p-10 flex flex-col items-center justify-center">
                                ...
                                <section className="mt-5 pb-28">
                                    <div className="mx-auto max-w-2xl">
                                        ...
                                        <div className="flex items-center justify-center gap-2 mb-5 flex-wrap">
                                            {quizQuestions.map((q, idx) => {
                                                ...
                                                return (
                                                    <span
                                                        key={idx}
                                                        className={`inline-flex items-center justify-center w-8 h-8 rounded-lg text-xs font-black transition-colors ${isCurrent ? "bg-primary text-primary-foreground shadow-md shadow-primary/30" : isAnswered ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}>
                                                        {idx + 1}
                                                    </span>
                                                );
                                            })}
                                        ...
                                        <div
                                            ...
                                            <div
                                                className="bg-gradient-to-r from-primary/8 to-secondary/5 px-6 py-4 border-b border-border flex items-center gap-3">
                                                <span
                                                    className="inline-flex items-center justify-center w-9 h-9 rounded-xl bg-primary text-primary-foreground text-base font-black shrink-0">
                                                    {quizCurrentIndex + 1}
                                                </span>
                                                <div className="flex-1 min-w-0">
                                                    <p
                                                        className="text-[11px] font-black text-muted-foreground uppercase tracking-wider">单选题</p>
                                                    ...
                                                    {currentQ.options.map(option => {
                                                        ...
                                                        return (
                                                            <button
                                                                ...
                                                                <span
                                                                    className={`shrink-0 mt-0.5 w-8 h-8 rounded-lg border-2 flex items-center justify-center text-sm font-black transition-colors ${optionLetterBg}`}>
                                                                    {option.id}
                                                                </span>
                                                                <span
                                                                    className={`text-sm leading-6 pt-0.5 ${isSelected ? "text-foreground font-bold" : "text-foreground/90"}`}>{option.text}</span>
                                                            </button>
                                                        );
                                                    })}
                                                </div>
                                        ...
                                        <div className="mt-5 flex items-center justify-between gap-3">
                                            <button
                                                onClick={() => {
                                                    if (quizCurrentIndex > 0) {
                                                        const updatedAnswers = quizSelectedOption ? {
                                                            ...quizAnswers,
                                                            [currentQ.question_id]: quizSelectedOption
                                                        } : quizAnswers;
                                            ...
                                            <button
                                                ...
                                                disabled={!quizSelectedOption || busy}
                                                ...
                                                {busy ? <><LoaderCircle className="w-4 h-4 animate-spin" />提交中…</> : isLast ? <>提交答卷 <Check className="w-4 h-4" /></> : <>下一题 <ArrowRight className="w-4 h-4" /></>}
                                            </button>
                                        </div>
                                    </div>
                                </section>
                            ...
                            return (
                                <section className="mt-5 pb-28">
                                    <div className="mx-auto max-w-2xl">
                                        {}
                                        <div
                                            className="mb-5 rounded-2xl bg-card border border-border shadow-card p-4 flex items-center gap-4">
                                            <div
                                                className={`shrink-0 w-14 h-14 rounded-xl flex items-center justify-center ${result.is_correct || pct >= 60 ? "bg-primary/10" : "bg-destructive/10"}`}>
                                                <span
                                                    className={`text-xl font-black ${pct >= 80 ? "text-success" : pct >= 60 ? "text-secondary" : "text-destructive"}`}>{pct}<span className="text-xs">分</span></span>
                                            </div>
                                            <div className="flex-1 min-w-0">
                                            ...
                                            <button
                                                onClick={() => setStage(2)}
                                                className="shrink-0 inline-flex items-center gap-1 px-4 py-2 rounded-lg bg-primary-container text-primary text-xs font-black border border-primary/30 hover:bg-primary hover:text-primary-foreground transition-colors">继续 <ArrowRight className="w-3.5 h-3.5" />
                                            </button>
                                        </div>
                                        {}
                                        <div className="flex items-center justify-center gap-2 mb-5 flex-wrap">
                                        ...
                                        <div
                                            className="rounded-2xl bg-card border border-border shadow-card overflow-hidden">
                                            <div
                                                className={`px-6 py-4 border-b border-border flex items-center gap-3 ${result.is_correct ? "bg-success/5" : "bg-destructive/5"}`}>
                                                <span
                                                    className={`shrink-0 inline-flex items-center gap-1 px-3 py-1.5 rounded-md text-xs font-black ${result.is_correct ? "bg-success text-success-foreground" : "bg-destructive text-destructive-foreground"}`}>
                                            ...
                                            <div className="p-6">
                                                ...
                                                <div className="mt-5 space-y-2.5">
                                                    {q.options.map(option => {
                                                        ...
                                                        return (
                                                            <div
                                                                key={option.id}
                                                                className={`rounded-xl border-2 px-4 py-3.5 flex items-start gap-3 ${optStyle}`}>
                                                                <span
                                                                    className={`shrink-0 mt-0.5 w-8 h-8 rounded-lg border-2 flex items-center justify-center text-sm font-black ${letterStyle}`}>{option.id}</span>
                                                ...
                                                <div
                                                    className="mt-5 rounded-xl bg-primary-container/30 border border-primary/15 p-4">
                                                ...
                                                {!result.is_correct && <div
                                                    className="mt-3 rounded-xl bg-destructive/5 border border-destructive/20 p-4">
                                        ...
                                        <div className="mt-5 flex items-center justify-between gap-3">
                                            ...
                                            {onReviewLast ? <button
                                                onClick={() => setStage(2)}
                                                className="inline-flex items-center gap-1.5 px-5 py-3 rounded-xl bg-primary-container text-primary text-sm font-black border border-primary/30 hover:bg-primary hover:text-primary-foreground transition-colors">进入分步推演 <ArrowRight className="w-4 h-4" />
                                            </button> : <button
                                                onClick={() => setReviewIndex(reviewIndex + 1)}
                                                className="inline-flex items-center gap-1.5 px-5 py-3 rounded-xl bg-primary-container text-primary text-sm font-black border border-primary/30 hover:bg-primary hover:text-primary-foreground transition-colors">下一题 <ArrowRight className="w-4 h-4" />
                                            </button>}
                                        </div>
                                    </div>
                                </section>
                    ...
                    {stage === 2 && <section
                        className="student-stage-panel student-stage-panel-simulation mt-4 rounded-2xl bg-card/90 border border-border/60 shadow-card p-5">
                        <div className="flex flex-wrap items-center gap-3">
                            <p
                                className="text-xs font-black text-primary-foreground bg-secondary border border-secondary/40 rounded-full px-2.5 py-1 inline-flex items-center gap-1"><PenLine className="w-3 h-3" />推演级 · 分步描述</p>
                        ...
                        <div className="mt-2.5 h-1.5 rounded-full bg-muted overflow-hidden"><div
                                className="h-full rounded-full bg-secondary transition-all"
                                style={{
                                    width: `${describedCount / step.keyPoints.length * 100}%`
                                }} /></div>
                        <p className="mt-2 text-right text-[11px] font-bold text-muted-foreground">{draftNotice}</p>
                        {preview && <div className="mt-3 flex justify-end">
                            <button
                                type="button"
                                onClick={() => setDescs(Object.fromEntries(step.keyPoints.map(point => [
                                    point.id,
                                    `示例描述：${point.label}。${point.hints[0]}${point.hints[1] ? `补充：${point.hints[1]}` : ""}`
                                ])))}
                            ...
                            {step.keyPoints.map((point, index) => {
                                ...
                                return (
                                    <div key={point.id} className="rounded-xl border border-border bg-card p-3">
                                        <div className="flex flex-wrap items-center gap-2">
                                            <span
                                                className="w-6 h-6 shrink-0 rounded-full bg-secondary-container text-secondary text-xs font-black flex items-center justify-center">{index + 1}</span>
                                            <p className="text-sm font-bold">{point.label}</p>
                                            <p className="text-xs text-muted-foreground">—— {point.hints[0]}</p>
                                            <span
                                                className={`ml-auto text-xs font-bold ${described ? "text-success" : "text-muted-foreground"}`}>{described ? "已描述 ✓" : "未描述"}</span>
                                            <button
                                                type="button"
                                                onClick={() => setHintLevel(value => ({
                                                    ...value,
                                                    [point.id]: Math.min(hints + 1, 3)
                                                }))}
                                                disabled={hints >= 3}
                                                className={`text-xs px-2 py-1 rounded-full border transition-colors ${hints >= 3 ? "border-border text-muted-foreground cursor-not-allowed" : "border-border text-muted-foreground hover:bg-muted cursor-pointer"}`}>提示 {hints}/3
                                                                                                                      </button>
                                        </div>
                                        {hints > 0 && <ul
                                            className="mt-2 space-y-1 text-xs text-muted-foreground bg-muted/60 rounded-lg p-2.5">
                                            {point.hints.slice(0, hints).map(
                                                hint => <li key={hint} className="flex gap-1.5"><span className="text-primary font-black shrink-0">›</span>{hint}</li>
                                            )}
                                        </ul>}
                                        <textarea
                                            value={text}
                                            onChange={event => setDescs(value => ({
                                                ...value,
                                        ...
                                        <div className="mt-1 flex items-center justify-between text-[11px]">
                                            <span
                                                className={described ? "text-success font-bold" : "text-muted-foreground"}>{described ? "✓ 已达字数要求" : `还差 ${MIN_DESC_LENGTH - text.trim().length} 字`}</span>
                        ...
                        {error && <p
                            className="mt-3 text-xs font-bold text-destructive flex items-center gap-1.5"><CircleAlert className="w-3.5 h-3.5" />{error}</p>}
