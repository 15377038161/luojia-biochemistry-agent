...
const baseExperimentSteps: Array<Omit<ExperimentStep, 'sopParameters' | 'safetyNotes' | 'decisionTree' | 'instruments' | 'scientificPractice'>> = [
  {
    ...
    principle: '重组克隆的核心原理是同源重组：引物在退火区之外加上与线性化载体末端匹配的15–25 bp同源臂，PCR扩增后的片段与线性化载体在重组酶作用下通过同源区完成无缝拼接。pET-28a(+)是常用的T7表达载体，携带T7启动子、lacO操纵序列、N端His标签和卡那霉素抗性；T7启动子只被T7 RNA聚合酶识别，而BL21(DE3)基因组整合了T7 RNA聚合酶基因，二者配合实现高效诱导表达。引物设计需兼顾长度(18–25 bp)、GC含量(40–60%)、Tm值(55–65°C且上下游接近)，并检查二聚体和发夹结构，以保证PCR特异性和扩增效率。',
    keyPoints: [
  ...
  },
  {
    ...
    principle: '无缝重组利用DNA片段两端15–25 bp的同源序列，在重组酶催化下完成链交换和连接，无需限制性内切酶和连接酶，且不引入额外碱基。保持正确方向和读码框至关重要：插入片段必须以正向且与载体读码框对齐的方式重组，否则His标签与目的蛋白会移码或截断。重组产物转入DH5α克隆菌株(而非表达菌株)进行扩增和筛选，因DH5α缺乏T7 RNA聚合酶基因，避免了表达载体的提前泄露表达。卡那霉素筛选去除未转化的细菌，阳性克隆通过菌落PCR或诊断酶切初步确认，最终由测序验证序列和方向。',
    keyPoints: [
  ...
  },
  {
    ...
    principle: 'BL21(DE3)基因组中整合了T7 RNA聚合酶基因，其表达受lacUV5启动子控制。IPTG(异丙基-β-D-硫代半乳糖苷)是乳糖类似物，它结合并灭活LacI阻遏蛋白，解除对lacUV5的抑制，使T7 RNA聚合酶转录，进而识别pET载体上的T7启动子驱动目的基因高水平转录。pET-28a(+)还携带lacI基因，在未加IPTG时维持基础表达极低(本底泄露抑制)。诱导参数(浓度、温度、时间)影响表达量和可溶性：高浓度低温长时间倾向于可溶表达，高浓度高温短时间易形成包涵体。需设置未诱导对照和空载体对照以排除内源蛋白干扰。',
    keyPoints: [
  ...
  },
  {
    ...
    principle: 'SDS-PAGE(十二烷基硫酸钠-聚丙烯酰胺凝胶电泳)的原理是：SDS与蛋白质按约1.4:1的质量比结合，使蛋白带上大量负电荷，掩盖了自身电荷差异；还原剂(如DTT/β-巯基乙醇)打开二硫键，使蛋白完全变性为线性分子。在电场中，变性蛋白的迁移率仅取决于分子量大小，通过聚丙烯酰胺凝胶的分子筛效应实现分离。通过比较裂解上清(可溶组分)和沉淀(不溶组分)中的目标条带，可以判断目的蛋白是可溶表达还是形成包涵体——这是决定后续纯化路线的关键依据。Marker提供分子量参照，未诱导和诱导对照用于确认表达差异。',
    keyPoints: [
  ...
  },
  {
    ...
    principle: 'His标签亲和层析(Ni-NTA)利用6×His标签中组氨酸的咪唑基团与固定在树脂上的Ni²⁺离子发生配位结合，实现目标蛋白的特异性捕获。杂蛋白不含密集的组氨酸，结合力弱，可被低浓度咪唑洗涤去除；而目标蛋白需高浓度咪唑竞争置换洗脱——咪唑与组氨酸咪唑环结构相似，高浓度时竞争结合Ni²⁺位点，将目标蛋白洗下。纯化路线的选择必须依据上一步的可溶性判断：可溶表达走天然条件下的Ni-NTA(保持蛋白活性)，包涵体则需变性(8 M尿素/盐酸胍)条件下纯化后尝试复性。Buffer体系的pH(7.4–8.0)、盐浓度(NaCl 300–500 mM)和低温(4°C)影响结合稳定性和非特异吸附。',
    keyPoints: [
  ...
  },
  {
    ...
    principle: 'Ni-NTA亲和层析的操作流程为：先用结合缓冲液(含低浓度咪唑20–40 mM以减少杂蛋白非特异吸附)平衡树脂柱；上样时目标蛋白的His标签与Ni²⁺配位结合，杂蛋白和未结合蛋白流出(流穿组分)；洗涤缓冲液(中等咪唑浓度40–60 mM)进一步去除弱结合杂蛋白；最后用洗脱缓冲液(高咪唑250–500 mM)竞争置换目标蛋白。保留各组分(上样、流穿、洗涤、各洗脱份)的目的是追踪蛋白流向、定位损失环节并评估纯化效率——若目标蛋白出现在流穿中，说明标签未暴露或柱超载；若洗脱不下，可能是结合过强或咪唑浓度不足。透析去除咪唑以恢复蛋白活性或满足后续应用。',
    keyPoints: [
  ...
  },
  {
    ...
    principle: '蛋白纯度验证以SDS-PAGE为主：在等量上样、充分染色和适当曝光条件下，目标条带泳道的单一性和杂带比例反映纯度水平。ImageJ等密度分析软件可对凝胶图像进行条带灰度定量，计算目标条带占总蛋白的百分比。Western Blot利用抗His标签或抗EGFP抗体进行免疫印迹，可确认目标条带的身份特异性——即该条带确实是目的蛋白而非大小相近的杂蛋白。单一目标条带不等于绝对纯度，需综合评估杂带数量与比例、染色背景以及目标蛋白的预期分子量(EGFP约27 kDa + His标签)。达标标准通常为目标条带纯度>90%且无明显降解或杂带。',
    keyPoints: [
  ...
  },
  {
    ...
    principle: '蛋白浓度测定常用三种方法：BCA法基于蛋白在碱性环境中将Cu²⁺还原为Cu⁺，BCA试剂与Cu⁺显色，562 nm吸光度与蛋白浓度成正比，受DTT和螯合剂干扰；Bradford法基于考马斯亮蓝G-250与蛋白结合后颜色由棕变蓝，595 nm测定，受表面活性剂(SDS)干扰；A280紫外吸收法利用芳香族氨基酸(色氨酸/酪氨酸)的紫外吸收，需已知消光系数，受核酸污染影响。所有定量方法都依赖标准品制作标准曲线：设置梯度浓度标准品(如0、0.125、0.25、0.5、1.0、2.0 mg/mL BSA)，测定吸光度后线性拟合，要求R²≥0.99。样品需在标准曲线线性范围内，超出范围需稀释。全流程回顾需将八步的因果关系串联：序列→引物→载体→转化→诱导→电泳→纯化→定量，任何一步的错误都会向下游传递。',
    keyPoints: [
