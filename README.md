...
- 教师智能体：`/teacher/dashboard` 只保留教学信号；学生列表使用快速抽屉，`/teacher/students/[sessionId]` 提供五分栏完整档案；`/teacher/content/[stepId]` 按理论、SOP安全、评分Gate、资料设备、预览发布分屏编辑。
- 模型网关：服务端 OpenAI Chat 兼容调用；`qwen3.8-max` 用于严肃评测、报告、教师分析和图片理解，`deepseek-v4-flash-0731` 用于高频助教和随机出题。深度思考字段不兼容时自动退回模型默认推理。
- 随机题库：单页单题暂存，最终确认后统一批改；先抽取本人未做题，题池不足才由 AI 生成、校验并入池，生成失败明确提示重试，不重复旧题或提前返回答案。
